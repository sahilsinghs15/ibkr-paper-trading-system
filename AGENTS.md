...
One FastAPI process executes TradingView webhooks through Model Blue → RMS (checks 2/3/4/7/8) → basket OMS → IBKR TWS, with Postgres persistence. The Vite React app under `frontend/` is a live PnL dashboard plus a **Settings** page for RMS limits and capital allocation. It is served by a separate demo SSE process on port **8010** (not by `app.main`).
...
- `POST /api/webhooks/tradingview` **does execute** trades when `order_manager` is on `app.state` (capture to disk **and** pipeline).
...
- Default IBKR port is `7497` (paper TWS). Live ports (`7496` / `4001`) are **not** rejected by code.
...
- Prefer declared frontend deps; dashboard uses SSE on `demo_streaming`, not WebSocket on `app.main`.
...
| Path | Why |
|------|-----|
| `backend/main.py` | Prints `"Hello from backend!"` — not the FastAPI app |
| `backend/scripts/test_tws_broker.py` | Imports missing `app.broker.ibkr.ibkr_broker.IBKRBroker` |
| Unused schemas in `app/schemas/api_schemas.py` | `PlaceOrderRequest`, `ModifyOrderRequest`, `BrokerStatusResponse`, `PositionSchema`, `MarginSchema`, `SignalSchema` — no matching routes |
| `backend/POSTMAN_API_TESTING_GUIDE.md` | Historical; lists APIs that are not in routers |
