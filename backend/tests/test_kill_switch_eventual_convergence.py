...
from app.db.models.kill_switch import KillSwitchOperationModel, KILL_SWITCH_STATUS_UNRESOLVED, KILL_SWITCH_STATUS_COMPLETE
...
async def test_execution_arrives_after_tier2_then_retry_converges(session_factory: async_sessionmaker[AsyncSession]):
    ...
    retried = await svc.retry_unresolved_operations()
    assert retried == 1
...
@pytest.mark.asyncio
async def test_retry_is_idempotent_duplicate_execution(session_factory):
    ...
    async with session_factory() as s, s.begin():
        ...
        await repo.upsert(type("E", (), {"exec_id": f"exec-dup-{trade_id}", "order_id": o1.id, "account_id": acc_id, "internal_order_id": o1.internal_order_id, "broker_order_id": None, "symbol": "AAPL", "side": "SELL", "quantity": Decimal("10"), "price": Decimal("155"), "commission": None, "commission_currency": None, "realized_pnl": None, "perm_id": None, "executed_at": None})(), order_id=o1.id, account_id=acc_id)
