...
import { fetchEventJournal } from '../../api/eventJournalApi'
import type { EventJournalItem } from '../../types/eventJournal'
...
/**
 * Machine/system event journal (event_log): RMS, OMS, reconcile, service events.
 * This is operational telemetry, not the operator audit trail.
 */
export function SystemEventJournal() {
  ...
  const [items, setItems] = useState<EventJournalItem[]>([])
  const [total, setTotal] = useState(0)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [selectedLog, setSelectedLog] = useState<EventJournalItem | null>(null)
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const res = await fetchEventJournal({
        ...
        if (active) {
          setError(err instanceof Error ? err.message : 'Failed to load the system event journal')
        }
      } finally {
        if (active) {
          setLoading(false)
