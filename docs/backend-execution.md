...
- Pair / account auto-exit: `RiskExitMonitor` (`services/risk_exit_monitor.py`) evaluates `positions.target` / `stop` / `time_limit` (and units) plus account daily session PnL. Pair arm flag is `positions.exit_automation_enabled` (seeded from the allocation at open; Settings toggle also propagates to currently OPEN rows). Pair hits call `SinglePairCloseService`; account hits arm the kill switch (`requested_by=auto_risk`). Ships off: `RISK_EXIT_MONITOR_ENABLED=false`, per-row `exit_automation_enabled` / `account_risk_enabled` default false. Shadow mode (`RISK_EXIT_SHADOW_MODE`) emits events without placing orders. RTH-only; stale marks skip stop/target (time-limit still fires). Open-pair thresholds can be edited via `PATCH /api/v1/config/accounts/{id}/positions/{trade_id}/exits`.
